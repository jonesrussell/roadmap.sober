...
func Button(canvas *svg.SVG, index int, title string, centerX int, id string) (middleTopX, middleTopY, middleBottomX, middleBottomY int) {
	...
	canvas.Group(fmt.Sprintf(`id="%s" style="transform:translate(%fpx, %fpx); cursor: pointer;" hx-get="/roadmap/%s" hx-target="#slide-over-content" hx-swap="innerHTML" onclick="dispatchOpenDrawerEvent();"`, id, float64(groupCenterX), rectY, id))
